...
def RecuperaGrupos(grupos, preguntas, grupo2):
    ...
    for NomGrupo in grupos['CodigoGrupo'].values:
        ...
        Preguntas = dcc.Dropdown(
            id={'index': desgru+'pre1', 'type':'din_pregu'},
            options=[
...
##Diccionario con los datos calculados de estudiantes por CCAA
with open("data/EstuCal.dat", "rb") as f:
    DatCal= pickle.load(f)

##Diccionario con los datos calculados de estudiantes por Población
with open("data/EstuCal2.dat", "rb") as f:
    DatCal2= pickle.load(f)

##Diccionario con los datos calculados de estudiantes por tipo de escuela
with open("data/EstuCal3.dat", "rb") as f:
    DatCal3= pickle.load(f)

##Diccionario con los datos calculados de estudiantes por género
with open("data/EstuCal4.dat", "rb") as f:
     DatCal4 = pickle.load(f)
...
Cabecera = dbc.Card(children=[dbc.CardHeader(html.H3("Gráficas cuestionario del alumnado")),
                    dbc.CardBody(children=[
                        html.H5("En esta sección, se puede visualizar la mayoría de las preguntas que se han realizado a los cerca de 36.000 estudiantes de 15 años en diferentes centros educativos españoles."),html.P(style={"line-height":"1px"}),
                        ...
                        html.H5("    - Gráfico de tarta resumen, de todo el alumnado"),html.P(style={"line-height":"1px"}),
                        ...
                        html.H5("    - Pestaña con gráfico de barras desglosado por tipo de escuela"),html.P(style={"line-height":"1px"}),
                        html.H5("    - Pestaña con gráfico de barras desglosado por género")]
                    )])
...
tabs_1 = dbc.Col(className='pretty_container',width=4,children=[dcc.Tabs(value='tab1',
                children=[
                    ...
                    dcc.Tab(
                        label="Categorias 3",style=tab_style, selected_style=tab_selected_style,
                        value="tab3",
                        className="custom-tab",
                        selected_className="custom-tab--selected",
...
##Graficos
grafi1 = dbc.Col(width=12,children=[dcc.Graph(id="pie-chart")])
...
grafi2 = dbc.Col(width=12,children=[html.H3(children='Porcentaje alumnado por CCAA',
                    ...
                    dcc.RadioItems(id='countries-radio', value=1),
                    dcc.Graph(id="map-chart")])

grafi3x = html.Div(children=[dcc.Graph(id="bar-chartx")])





grafi1P = dbc.Col(width=12,children=[dcc.Graph(id="P1_pie-chart")])
grafi2P = dbc.Col(width=12,children=[dcc.Graph(id="P2_pie-chart")])

grafi3 = html.Div(children=[dcc.Graph(id="bar-chart")])

grafi3C1 = html.Div(children=[dcc.Graph(id="bar-chart1")])
grafi3C2 = html.Div(children=[dcc.Graph(id="bar-chart2")])
grafi3C3 = html.Div(children=[dcc.Graph(id="bar-chart3")])
grafi3C4 = html.Div(children=[dcc.Graph(id="bar-chart4")])

grafi1E = dbc.Col(width=12,children=[dcc.Graph(id="E1_pie-chart")])

grafi1G = dbc.Col(width=12,children=[dcc.Graph(id="G1_pie-chart")])

graficas = dbc.Col(className='pretty_container', width=7,
    children=dbc.Card(
        [dbc.CardHeader(className='texto_cab', children=['(',html.Label(id='vari'),')  ',html.H4(id='vari2')]),
...
graficasGen = dbc.Col(className='pretty_container', width=12,
    children=dbc.Card(
        [dbc.CardHeader(className='texto_cab', children=['Graficas por género']),
         grafi1G]))
TB = dcc.Tabs(children=[
                   dcc.Tab(id="EsGraficas1",style=tab_style, selected_style=tab_selected_style,
                           label="Gráficas por CCCAA", children=dbc.Row([graficasCCAA])),
                   dcc.Tab(id="EsGraficas2",style=tab_style, selected_style=tab_selected_style,
                           label="Gráficas por tipo de población", children=dbc.Row([graficasPob])),
                   dcc.Tab(id="EsGraficas3",style=tab_style, selected_style=tab_selected_style,
                           label="Gráficas por tipo de escuelas", children=dbc.Row([graficasEsc])),
                   dcc.Tab(id="EsGraficas4",style=tab_style, selected_style=tab_selected_style,
                           label="Gráficas por género", children=dbc.Row([graficasGen])),
                   ])
...
@app.callback(
    Output({'type': 'din_pregu', 'index': ALL}, 'value'),
    Output('vari', 'children'),Output('vari2', 'children'),
    Input( {'type': 'din_pregu', 'index': ALL}, 'value'),
    State('vari', 'children'),prevent_initial_call=True
    )
...
@app.callback(
    Output("pie-chart", "figure"),Output("P1_pie-chart", "figure"),Output("P2_pie-chart", "figure"),
    Output("E1_pie-chart", "figure"), Output("G1_pie-chart", "figure"),
    Output('countries-radio', 'options'), Output('countries-radio', 'value'),
    [
     Input( 'vari', 'children'),
...
def update_charts(variable):
    ...
    df3 = DatCal4.get(variable)
    df3 = df3.replace({"ST004D01T": tip_genero}).sort_values(variable)
    figp4 = px.bar(df3, x=variable, y=0, color=variable, barmode="group",facet_col='ST004D01T')

    return fig, figp1, figp2, figp3, figp4, options, df3.iloc[0][variable]
...
@app.callback(
    Output("bar-chart", "figure"), Output("bar-chart1", "figure"), Output("bar-chart2", "figure"), Output("bar-chart3", "figure"),
    Output("bar-chart4", "figure"),Output("bar-chartx", "figure"),
    [
        Input( 'vari', 'children')
    ],prevent_initial_call=True
    )
