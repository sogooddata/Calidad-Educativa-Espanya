...
def RecuperaGrupos(grupos, preguntas, grupo2):
    ...
    for NomGrupo in grupos['CodigoGrupo'].values:
        ...
        Nombres.append(html.Label(id=desgru+'Esc_', children=desgru))

        Preguntas = dcc.Dropdown(
            id={'index': desgru+'Esc_', 'type':'Esc_din_pregu'},
            options=[
...
Cabecera = dbc.Card(children=[dbc.CardHeader(html.H3("Gráficas cuestionario de Centros Educativos")),
                    dbc.CardBody(children=[
                        html.H5("En esta sección, se podrá visualizar la mayoría de las preguntas que se han realizado a los 1036 centros educativos españoles seleccionadas en el Informe PISA."),html.P(style={"line-height":"1px"}),
                        ...
                        html.H5("    - Gráfico de tarta resumen, de todos los centros educativos"),html.P(style={"line-height":"1px"}),
...
preguntas = pd.read_csv("data/GruPreguntasESC.csv", na_values = ["NaN", "NaN"], sep = ";")

grupos = pd.read_csv("data/GruposESC.csv", na_values = ["NaN", "NaN"], sep = ";")
...
##Diccionario con los datos calculados de escuelas por CCAA
with open("data/EscCal.dat", "rb") as f:
    DatCal= pickle.load(f)

##Diccionario con los datos calculados de escuelas por población
with open("data/EscCal2.dat", "rb") as f:
    DatCal2= pickle.load(f)

##Diccionario con los datos calculados de escuelas por tipo de escuela
with open("data/EscCal3.dat", "rb") as f:
...
##Graficos
grafi1 = dbc.Col(width=12,children=[dcc.Graph(id="Esc_pie-chart")])
...
grafi2 = dbc.Col(width=12,children=[html.H3(children='Porcentaje alumnado por CCAA',
                    ...
                    dcc.RadioItems(id='countries-radioe', value=1),
                    dcc.Graph(id="map-charte")])

grafi3x = html.Div(children=[dcc.Graph(id="bar-chartxe")])



grafi1P = dbc.Col(width=12,children=[dcc.Graph(id="P1Esc_pie-chart")])
grafi2P = dbc.Col(width=12,children=[dcc.Graph(id="P2Esc_pie-chart")])


#grafi2 = dbc.Col(width=6,children=[html.H3(children='Porcentaje alumnado por CCAA',
#                    style={'color': 'white', 'fontSize': 24, 'background-color': 'black', ' text-align': 'center'}),
#                    dcc.RadioItems(id='Esc_countries-radio', value=1),
#                    dcc.Graph(id="Esc_map-chart")])

grafi3 = html.Div(children=[dcc.Graph(id="Esc_bar-chart")])
grafi3C1 = html.Div(children=[dcc.Graph(id="Esc_bar-chart1")])
grafi3C2 = html.Div(children=[dcc.Graph(id="Esc_bar-chart2")])
grafi3C3 = html.Div(children=[dcc.Graph(id="Esc_bar-chart3")])
grafi3C4 = html.Div(children=[dcc.Graph(id="Esc_bar-chart4")])

grafi1E = dbc.Col(width=12,children=[dcc.Graph(id="E1Esc_pie-chart")])

graficas = dbc.Col(className='pretty_container', width=7,
    children=dbc.Card(
        [dbc.CardHeader(className='texto_cab', children=['(',html.Label(id='Esc_vari'),')  ',html.H4(id='Esc_vari2')]),
...
TB = dcc.Tabs(children=[
                   dcc.Tab(id="EGraficas1",style=tab_style, selected_style=tab_selected_style,
                           label="Gráficas por CCCAA", children=dbc.Row([graficasCCAA])),
                   dcc.Tab(id="EGraficas2",style=tab_style, selected_style=tab_selected_style,
                           label="Gráficas por tipo de población", children=dbc.Row([graficasPob])),
                   dcc.Tab(id="EGraficas3",style=tab_style, selected_style=tab_selected_style,
...
@app.callback(
    Output({'type': 'Esc_din_pregu', 'index': ALL}, 'value'),
    Output('Esc_vari', 'children'),Output('Esc_vari2', 'children'),
    Input( {'type': 'Esc_din_pregu', 'index': ALL}, 'value'),
    State('Esc_vari', 'children'),prevent_initial_call=True
    )
...
@app.callback(
    Output("Esc_pie-chart", "figure"),Output("P1Esc_pie-chart", "figure"),Output("P2Esc_pie-chart", "figure"),
    Output("E1Esc_pie-chart", "figure"),
    Output('countries-radioe', 'options'), Output('countries-radioe', 'value'),
    [
     Input( 'Esc_vari', 'children'),
...
@app.callback(
    Output("map-charte", "figure"),
    [
        Input( 'Esc_vari', 'children'),
        Input('countries-radioe', 'value')
    ],prevent_initial_call=True
)
...
@app.callback(
    Output("Esc_bar-chart", "figure"), Output("Esc_bar-chart1", "figure"), Output("Esc_bar-chart2", "figure"), Output("Esc_bar-chart3", "figure"),
    Output("Esc_bar-chart4", "figure"),Output("bar-chartxe", "figure"),
    [
        Input( 'Esc_vari', 'children')
    ],prevent_initial_call=True
    )
